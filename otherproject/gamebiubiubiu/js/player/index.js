...
const WEAPON_SWITCH_INTERVAL = 300; // 5秒切换武器 (5 * 60帧)
...
export default class Player extends Animation {
  ...
  shoot() {
    if (this.isLaserMode) {
      ...
      laser.init(this.x + this.width / 2 - laser.width / 2, this.y - 10, 15);
      GameGlobal.databus.bullets.push(laser);
    } else {
      // 霰弹枪模式：发射5发整齐排列的子弹
      const bulletSpeed = 12;
      const bulletCount = 5;
      const spacing = 18; // 子弹间距
      ...
      for (let i = 0; i < bulletCount; i++) {
        ...
        bullet.init(startX + i * spacing, this.y - 10, bulletSpeed);
  ...
  update() {
    ...
    // 武器切换计时器
    this.weaponTimer++;
    if (this.weaponTimer >= WEAPON_SWITCH_INTERVAL) {
