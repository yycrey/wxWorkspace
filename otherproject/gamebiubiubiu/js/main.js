...
export default class Main {
  ...
  collisionDetection() {
    ...
    // 检测敌方子弹与玩家的碰撞
    for (let i = 0, il = GameGlobal.databus.enemyBullets.length; i < il; i++) {
  ...
  render() {
    ...
    GameGlobal.databus.bullets.forEach((item) => item.render(ctx)); // 绘制所有玩家子弹
    GameGlobal.databus.enemyBullets.forEach((item) => item.render(ctx)); // 绘制所有敌方子弹
